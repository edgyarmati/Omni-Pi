...
async function appendBullets(filePath: string, heading: string, bullets: string[]): Promise<void> {
  if (bullets.length === 0) {
    return;
  }
  ...
  const merged = [body, ...bullets.map((bullet) => `- ${bullet}`)].filter(Boolean).join("\n");
  await writeFile(filePath, content.replace(sectionRegex, `${prefix}${merged}\n`), "utf8");
}

export async function syncOmniMemory(rootDir: string, request: SyncRequest): Promise<void> {
  ...
  await appendBullets(sessionPath, "## Next handoff notes", request.nextHandoffNotes ?? []);

  if (request.decisions && request.decisions.length > 0) {
    const decisionLines = request.decisions.map((decision) => `Date: pending\n  - Decision: ${decision}\n  - Why: Captured during sync.\n  - Impact: To be refined.`);
