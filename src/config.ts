...
function parseModelTable(content: string, heading: string): Record<string, string> {
...
export async function writeConfig(rootDir: string, config: OmniConfig): Promise<void> {
  const configPath = path.join(rootDir, CONFIG_PATH);
  await mkdir(path.dirname(configPath), { recursive: true });
  await writeFile(configPath, renderConfigContent(config), "utf8");
}

export async function updateModelConfig(rootDir: string, agent: string, model: string): Promise<OmniConfig> {
