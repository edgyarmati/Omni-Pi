...
export function renderCompactStatus(state: OmniState): string[] {
  const phaseBar = Object.keys(phaseLabels).map((key) =>
    key === state.currentPhase ? `[${phaseLabels[key as OmniPhase]}]` : ` ${phaseLabels[key as OmniPhase]} `
  ).join(" > ");
  const lines = [`Omni-Pi  ${phaseBar}`];
...
export function renderMetrics(workerRuns: RunHistoryEntry[], expertRuns: RunHistoryEntry[]): string {
  const allRuns = [...workerRuns, ...expertRuns];
  if (allRuns.length === 0) {
    return "No agent run history available yet.";
  }

  function stats(runs: RunHistoryEntry[]): { total: number; successRate: string; avgDuration: string } {
    if (runs.length === 0) return { total: 0, successRate: "n/a", avgDuration: "n/a" };
  ...
  if (workerStats.total > 0) {
    lines.push(`  Worker: ${workerStats.total} runs, ${workerStats.successRate} success, avg ${workerStats.avgDuration}`);
  }
  if (expertStats.total > 0) {
    lines.push(`  Expert: ${expertStats.total} runs, ${expertStats.successRate} success, avg ${expertStats.avgDuration}`);
  }
