...
import { syncOmniMemory, type SyncRequest } from "./sync.js";
...
async function replaceSection(filePath: string, heading: string, lines: string[]): Promise<void> {
  const current = await readFile(filePath, "utf8");
  const escapedHeading = heading.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
  const sectionRegex = new RegExp(`(${escapedHeading}\\n\\n)([\\s\\S]*?)(?=\\n## |$)`, "u");
  const replacement = `$1${lines.join("\n")}\n`;
  const next = current.match(sectionRegex) ? current.replace(sectionRegex, replacement) : `${current.trimEnd()}\n\n${heading}\n\n${lines.join("\n")}\n`;
...
function buildSkillCandidates(repoSignals: Awaited<ReturnType<typeof detectRepoSignals>>): SkillCandidate[] {
  ...
  if (repoSignals.tools.includes("playwright") || repoSignals.tools.includes("cypress")) {
    candidates.push({
      ...
      reason: "The repository already has browser testing signals, so browser-oriented workflow helpers are useful.",
      confidence: "medium",
...
export async function initializeOmniProject(rootDir: string): Promise<InitResult> {
  ...
  await replaceSection(
    skillsPath,
    "## Recommended",
    skillCandidates.filter((candidate) => candidate.policy !== "auto-install").map(renderSkillDecision).concat(skillCandidates.every((candidate) => candidate.policy === "auto-install") ? ["- None yet"] : [])
  );
  ...
  if (!project.includes("## Repo Signals")) {
    await writeFile(projectPath, `${project.trimEnd()}\n\n## Repo Signals\n\n${signalSummary}\n`, "utf8");
  }

  if (installCommands.length > 0) {
    await appendSkillUsageNote(rootDir, `Planned install commands: ${installCommands.join(" ; ")}`);
  }

  await writeState(rootDir, {
    currentPhase: "understand",
    activeTask: "Initialize Omni-Pi",
    statusSummary: "Omni-Pi has created its project memory files and scanned the repository for useful signals.",
    blockers: [],
    nextStep: "Run /omni-plan to turn the current project context into a spec and first task slices."
  });
...
export async function planOmniProject(rootDir: string, brief: ConversationBrief): Promise<PlanResult> {
  ...
  await writeState(rootDir, {
    ...
    statusSummary: "Omni-Pi refreshed the spec, task slices, and verification plan.",
    blockers: [],
    nextStep: "Review the proposed tasks, then run /omni-work when you are ready to execute the next slice."
  });
...
export async function workOnOmniProject(rootDir: string, engine: WorkEngine): Promise<WorkExecutionResult> {
  ...
  if (result.kind === "completed" || result.kind === "expert_completed") {
    state = {
      ...
      blockers: [],
      nextStep: "Run /omni-status to review progress or /omni-work to continue with the next task."
    };
    ...
    state = {
      ...
      statusSummary: result.message,
      blockers: [],
      nextStep: "Run /omni-plan to refresh the task list if more work is needed."
    };
  }
...
export async function syncOmniProject(rootDir: string, request: SyncRequest): Promise<SyncResult> {
  ...
  const state: OmniState = {
    ...
    blockers: [],
    nextStep: "Run /omni-status to inspect the latest state or /omni-plan to refine the next slice."
  };
  await writeState(rootDir, state);
  return { state };
}
